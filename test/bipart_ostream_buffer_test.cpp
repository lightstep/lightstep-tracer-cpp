#include "../src/bipart_ostream_buffer.h"

#define CATCH_CONFIG_MAIN
#include <lightstep/catch2/catch.hpp>
using namespace lightstep;

TEST_CASE("BipartOstreamBuffer") {
  SECTION("Next returns false when there's no data attached.") {
    BipartOstreamBuffer buffer{nullptr, 0, nullptr, 0};
    void* data;
    int size;
  ...
  SECTION("Next returns the first memory buffer provided") {
    char data1[10];
    BipartOstreamBuffer buffer{data1, 10, nullptr, 0};
    void* data;
    int size;
    CHECK(buffer.Next(&data, &size));
  ...
  SECTION("Next returns the first, then second memory buffer if provided") {
    char data1[10];
    char data2[5];
    BipartOstreamBuffer buffer{data1, 10, data2, 5};
    void* data;
    int size;
    CHECK(buffer.Next(&data, &size));
    CHECK(static_cast<char*>(data) == data1);
