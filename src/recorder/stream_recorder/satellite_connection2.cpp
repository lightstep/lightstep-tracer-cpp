...
SatelliteConnection2::SatelliteConnection2(SatelliteStreamer2& streamer)
    : streamer_{streamer},
      ...
      reconnect_timer_{
          ...
          MakeTimerCallback<SatelliteConnection2,
                            &SatelliteConnection2::InitiateReconnect>(),
          static_cast<void*>(this)},
      graceful_shutdown_timeout_{
          streamer.event_base(), -1, 0,
          MakeTimerCallback<SatelliteConnection2,
                            &SatelliteConnection2::GracefulShutdownTimeout>(),
          static_cast<void*>(this)} {}

//--------------------------------------------------------------------------------------------------
// destructor
//--------------------------------------------------------------------------------------------------
SatelliteConnection2::~SatelliteConnection2() noexcept {
...
// Start
//--------------------------------------------------------------------------------------------------
void SatelliteConnection2::Start() noexcept { Connect(); }

//--------------------------------------------------------------------------------------------------
// Flush
//--------------------------------------------------------------------------------------------------
bool SatelliteConnection2::Flush() noexcept try {
...
// ready
//--------------------------------------------------------------------------------------------------
bool SatelliteConnection2::ready() const noexcept {
...
// Connect
//--------------------------------------------------------------------------------------------------
void SatelliteConnection2::Connect() noexcept try {
  ...
  read_event_ =
      Event{streamer_.event_base(), socket_.file_descriptor(), EV_READ,
            MakeEventCallback<SatelliteConnection2,
                              &SatelliteConnection2::OnReadable>(),
            static_cast<void*>(this)};
  ...
  write_event_ =
      Event{streamer_.event_base(), socket_.file_descriptor(), EV_WRITE,
            MakeEventCallback<SatelliteConnection2,
                              &SatelliteConnection2::OnWritable>(),
            static_cast<void*>(this)};
  write_event_.Add(streamer_.recorder_options().satellite_write_timeout);
} catch (const std::exception& e) {
...
// FreeSocket
//--------------------------------------------------------------------------------------------------
void SatelliteConnection2::FreeSocket() {
  socket_ = Socket{-1};
  read_event_ = Event{};
  write_event_ = Event{};
...
// HandleFailure
//--------------------------------------------------------------------------------------------------
void SatelliteConnection2::HandleFailure() noexcept try {
  FreeSocket();
  streamer_.event_base().OnTimeout(
      streamer_.recorder_options().satellite_failure_retry_period,
      MakeTimerCallback<SatelliteConnection2, &SatelliteConnection2::Connect>(),
      static_cast<void*>(this));
} catch (const std::exception& e) {
...
// ScheduleReconnect
//--------------------------------------------------------------------------------------------------
void SatelliteConnection2::ScheduleReconnect() {
...
// InitiateReconnect
//--------------------------------------------------------------------------------------------------
void SatelliteConnection2::InitiateReconnect() noexcept try {
  connection_stream_.Shutdown();
  if (writable_) {
    Flush();
  }
...
// Reconnect
//--------------------------------------------------------------------------------------------------
void SatelliteConnection2::Reconnect() noexcept try {
  FreeSocket();
  Connect();
} catch (const std::exception& e) {
...
//--------------------------------------------------------------------------------------------------
void SatelliteConnection2::GracefulShutdownTimeout() noexcept {
...
// OnReadable
//--------------------------------------------------------------------------------------------------
void SatelliteConnection2::OnReadable(int file_descriptor,
...
// OnWritable
//--------------------------------------------------------------------------------------------------
void SatelliteConnection2::OnWritable(int file_descriptor,
                                     short what) noexcept try {
  streamer_.logger().Info("Satellite file_descriptor ", file_descriptor,
                          " is writable");
...
// OnSocketError
//--------------------------------------------------------------------------------------------------
void SatelliteConnection2::OnSocketError() noexcept try {
  FreeSocket();
  Connect();
} catch (const std::exception& e) {
